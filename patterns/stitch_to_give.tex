...
\newcommand{\desc}{Ein Benutzer (der \sen) hat ein \data\ und will es mit einem weiteren Benutzer (dem \rec) bzw. einem \recdev\ in unmittelbarer Nähe teilen. Es soll vom \sendev\ auf das \recdev\ übertragen werden, sodass es anschließend auf beiden Geräten verfügbar ist.}

\newcommand{\solution}{Der \sen{} hält die Geräte aneinander, sodass sie sich berühren und die Bildschirme parallel zueinander sind. Er führt seinen Finger von einem Punkt auf dem Bildschirm des \sendev{}s über die Bildschirmgrenzen hinweg auf den Bildschirm des \recdev{}s, woraufhin das \data\ übertragen wird.}
...
Da es sich bei \textbf{Stitch to Give} um eine \gls{synchronegeste} handelt, werden nach dem Stitch einige relevante Daten (Richtung, Timestamp, Orientierung) der beiden Swipes verglichen. Entsprechen die Daten einem Stitch, so wird das \data\ versendet. 
}

\newcommand{\reactionRec}{
Auf dem \recdev{} werden die selben atomaren Interaktionen ausgeführt wie auf dem \sendev{}. Das Gerät sollte dem Benutzer Feedback über den eingegangenen Stitch und das Empfangen der Daten geben. 
}
...
\item[-] Da es sich bei Stitch to Give um eine synchrone Geste handelt, müssen die Geräte miteinander kommunizieren können, sodass die beiden Teile der Geste miteinander verglichen werden können.
\end{itemize}}
...
\newcommand{\validcontext}{Situationen bei denen ein \data\ genau einer Person in unmittelbarer Nähe gesendet werden soll (z.B. Projektmanager verteilt Tasks per Stitch an Mitarbeiter von seinem Tablet auf deren Smartphones).}
...
\newcommand{\notvalidcontext}{
\begin{itemize}
\item Versenden von Daten an entfernte Geräte/Personen.
\item Versenden von Daten an mehrere Geräte/Personen gleichzeitig.
\end{itemize}}
...
\newcommand{\metaphor}{}
\newcommand{\metaphordesc}{Hinüberschieben eines Dokuments auf einen Schreibtisch}
...
\caption{Benötigte Technlogien für das Stich To Give Pattern}
...
\newcommand{\versionhistory}{14.02.2017}
